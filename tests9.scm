...
(define (repeat x) (define x 5))
(repeat)
; expect (lambda (x) (define x 5))

(define x (define y x))
; expect Error: unknown identifier: x

(lambda (x y) (+ x y) 5)
; expect (lambda (x y) (begin + x y) 5))
...
(begin "I like puppies" "I like kittens")
; expect "I like kittens"

(define k (mu (a) (+ a b)))
;expect k
(define l (lambda (a b) (k (+ a a))))
;expect l
...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
  (define (construct-partition total max-pieces max-value part) ;creating a helper to construct a list of partitions
    (cond ((= total 0)                              ;base case
       (list part))
    ((or (= max-pieces 0) (< total 0) (<= max-value 0))  ;base case
       '())
    (else 
        (append (construct-partition 
        (- total max-value) (- max-pieces 1) max-value (append part (list max-value)))
        (construct-partition total max-pieces (- max-value 1) part )))))
  (construct-partition total max-pieces max-value '()))
...
(define (tree-sum-list tree)
  ...
  (if (null? tree)
    '()
    (append lst1 (cons ((entry tree)) (tree-sum-list (children tree))) nil))))
  (helper '()))
...
(define (tree-sum-list tree)
  ...
  (if (null? tree)
    '()
    (append (cons (entry tree) (tree-sum-list (children tree))))))

(define (tree-sum-list tree)
    (define (helper list_trees child_sums total)
      (if (null? list_trees)
        child_sums
        (begin (define tree (car list_trees))
          (if (null? (children tree))
             (append (append child_sums (cons (+ (entry tree) total) nil) 
              (helper (cdr list_trees) child_sums total)))
          (append (helper (children tree) child_sums (+ (entry tree) total)) 
                (helper (cdr list_trees) child_sums total))))))
        (helper (cons tree nil) '() 0))

(define (tree-sum-list tree)
    (define (helper list_trees total child-sums)
      (if (null? list_trees)
        child_sums
        (begin
          (define tree (car list_trees))
          (if (null? (children tree))
             (append (append child_sums (cons (+ (entry tree) total) nil) 
                    (helper (cdr list_trees) total child_sums)))
              (append (helper (children tree) (+ (entry tree) total) child_sums) 
                    (helper (cdr list_trees) total child_sums))))))
        (helper (cons tree nil) 0 nil))
  ; *** YOUR CODE HERE ***
