...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        "*** YOUR CODE HERE ***"
        call_frame = env.make_call_frame(procedure.formals,args)
...
def apply_primitive(procedure, args, env):
    ...
    try:
        ...
        raise SchemeError('not valid primative expression') 
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        frame = Frame(self)
        "*** YOUR CODE HERE ***"
        if len(formals) != len(vals): #raises error if the number of formals and vals differs
            raise SchemeError('Expected the same amount of arguments for formals and vals')
        while len(formals) != 0:
...
def do_lambda_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    body = vals.second.first #defining the lambda form
    if len(vals)>2:
...
def do_mu_form(vals):
    ...
    formals = vals[0]
    check_formals(formals)
    "*** YOUR CODE HERE ***" #defining the mu form (dynamic scoping)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        "*** YOUR CODE HERE ***"
        current_value = scheme_eval(vals[1], env)
        env.define(target, current_value)
        return target
    elif isinstance(target, Pair):
        "*** YOUR CODE HERE ***"
        if scheme_symbolp(target[0]):
            
            cur_val = do_lambda_form(Pair(target.second,vals.second),env)
            env.define(target.first,cur_val)
            return target.first
        else:
...
def do_let_form(vals, env):
    ...
    for bind in bindings:
        names = bind[0]
        values = Pair(names, scheme_eval(bind[1], env))
        new_env.define(names, values.second)
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    "*** YOUR CODE HERE ***"
    if len(vals) == 2 and scheme_eval(vals[0], env) == False:
        return okay
...
def do_and_form(vals, env):
    ...
    if len(vals) == 0:
        return True
    elem = 0
    while elem < len(vals)-1:   
        if not scheme_true(scheme_eval(vals[elem],env)):
            return False
        elem +=1
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            "*** YOUR CODE HERE ***"
            if len(clause.second) > 1 or clause.second: 
...
def check_formals(formals):
    ...
    if scheme_symbolp(formals):
        raise SchemeError('not a valid list of parameters')

    formals_list  = []
    
    for arg in formals:
        if arg in formals_list: #checking for repeated symbols
            raise SchemeError('repeated symbols')
        elif not scheme_symbolp(arg): #checking for invalid symbols
            raise SchemeError('not a valid symbol')
        formals_list.append(arg)
