...
def do_lambda_form(vals, env):
    ...
    if len(vals)>2:
        body = do_begin_form(vals.second, env)       
...
def do_define_form(vals, env):
    ...
    check_form(vals, 2)
    target = vals[0]
    if scheme_symbolp(target) is not None:
        check_form(vals, 2, 2)
        "*** YOUR CODE HERE ***"
        ...
        if scheme_symbolp(target):
            cur_val = do_lambda_form(Pair(Pair(target[1],nil),Pair(vals[1],nil)),env)
            env.define(target[0],cur_val)
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    "*** YOUR CODE HERE ***"
    evalu = scheme_eval(vals[0], env)
    if len(vals) == 2 and evalu == False:
        return okay
    if evalu: 
        return vals[1]
    else:
